...
const postProductCart = async (req) => { // agrega un producto al cart
  try {
    const { id } = req.params;
  const username = req.user.email;
  const user = await Usuarios.find({ email: username });
...
const postActualizarCarrito = async (req) => { // elimina un producto del cart
  try {
    const { id } = req.params;
    const username = req.user.email;
    const user = await Usuarios.find({ email: username });
...
const postFinalizarCompra = async (req) => { // envia el mail y msg una vez finalizada la compra
  try {
    const vaciarCarrito = [];
    const username = req.user.email;
    const user = await Usuarios.find({ email: username });
